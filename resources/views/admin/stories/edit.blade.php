...
@section('content')
    <div class="d-flex flex-column flex-column-fluid">
        ...
        <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
            ...
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                ...
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <!--begin::Title-->
                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">@lang('common.edit_story')</h1>
                    <!--end::Title-->
                    <!--begin::Breadcrumb-->
        ...
        <div id="kt_app_content" class="app-content flex-column-fluid">
            ...
            <div id="kt_app_content_container" class="app-container container-xxl">
                <form id="kt_form_story" action="{{ route('admin.stories.update', $story->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card">
                        <div class="card-header border-0 pt-6">
                            <h3 class="card-title align-items-start flex-column">
                                <span class="card-label fw-bold fs-3 mb-1">@lang('common.story_details')</span>
                                <span class="text-muted mt-1 fw-semibold fs-7">@lang('common.edit_story_content')</span>
                            </h3>
                        </div>
                        <div class="card-body">
                            <!-- Tabs -->
                                ...
                                <div class="tab-pane fade show active" id="kt_tab_basic" role="tabpanel">
                                    <div class="row">
                                        <div class="col-md-6 mb-5">
                                            ...
                                            <input type="text" name="category_en" class="form-control form-control-solid" value="{{ $story->category_en }}" required/>
                                        </div>
                                        <div class="col-md-6 mb-5">
                                            <label class="required fw-semibold fs-6 mb-2">@lang('common.category_arabic')</label>
                                            <input type="text" name="category_ar" class="form-control form-control-solid" value="{{ $story->category_ar }}" required/>
                                        </div>
                                        <div class="col-md-6 mb-5">
                                            <label class="fw-semibold fs-6 mb-2">@lang('common.slug')</label>
                                            <input type="text" name="slug" class="form-control form-control-solid" value="{{ $story->slug }}"/>
                                            <div class="form-text">@lang('common.auto_generate_slug')</div>
                                        </div>
                                        <div class="col-md-6 mb-5">
                                            <label class="fw-semibold fs-6 mb-2">@lang('common.order')</label>
                                            <input type="number" name="order" class="form-control form-control-solid" value="{{ $story->order }}" min="0"/>
                                        </div>
                                        <div class="col-md-6 mb-5">
                                            <label class="required fw-semibold fs-6 mb-2">@lang('common.status')</label>
                                            <select name="status" class="form-select form-select-solid" required>
                                                <option value="draft" {{ $story->status === 'draft' ? 'selected' : '' }}>@lang('common.draft')</option>
                                                <option value="published" {{ $story->status === 'published' ? 'selected' : '' }}>@lang('common.published')</option>
                                            </select>
                                        </div>
                                        <div class="col-md-6 mb-5">
                                            <label class="fw-semibold fs-6 mb-2">@lang('common.published_at')</label>
                                            <input type="datetime-local" name="published_at" class="form-control form-control-solid" value="{{ $story->published_at ? $story->published_at->format('Y-m-d\TH:i') : '' }}"/>
                                        </div>
                                        <div class="col-12 mb-5">
                                            ...
                                            <input type="text" name="tags" class="form-control form-control-solid" 
                                                   value="{{ is_array($story->tags) ? implode(', ', $story->tags) : $story->tags }}" 
                                    ...
                                    <div class="row">
                                        <div class="col-12 mb-10">
                                            ...
                                            <div class="image-upload-wrapper">
                                                <input type="hidden" name="featured_image" id="featured_image_path" class="image-path-input" value="{{ $story->featured_image }}">
                                                <div class="image-preview mb-3" style="max-width: 400px;">
                                                    @if($story->featured_image)
                                                        <img src="{{ asset('storage/' . $story->featured_image) }}" alt="Preview" class="img-thumbnail" style="max-width: 100%; height: auto;">
                                                    @else
                                                ...
                                                <button type="button" 
                                                        class="btn btn-sm btn-light-primary upload-story-image-btn"
                                        ...
                                        {{-- <div class="col-12 mb-5">
                                            ...
                                            <div id="gallery_images_wrapper">
                                                @if($story->images && is_array($story->images) && count($story->images) > 0)
    ...
    <!-- Image Upload Modal -->
    <div class="modal fade" id="kt_modal_story_image_upload" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="kt_form_story_image_upload" enctype="multipart/form-data">
                        <input type="file" name="image" id="story_image_input" accept="image/*" class="form-control mb-5" required>
                        <div class="image-preview-upload mb-5" style="display: none;">
                            <img id="preview_story_image" src="" alt="Preview" class="img-thumbnail" style="max-width: 100%; height: auto;">
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="upload_story_image_btn">@lang('common.upload')</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.getElementById('story_image_input')?.addEventListener('change', function(e) {
        const file = e.target.files[0];
        if (file) {
            const reader = new FileReader();
            reader.onload = function(e) {
                document.getElementById('preview_story_image').src = e.target.result;
    ...
    document.querySelectorAll('.upload-story-image-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            const imageType = this.getAttribute('data-image-type');
            document.getElementById('upload_image_type').value = imageType;
            
            // Reset form
            document.getElementById('kt_form_story_image_upload').reset();
            document.querySelector('.image-preview-upload').style.display = 'none';
            
            const modal = new bootstrap.Modal(document.getElementById('kt_modal_story_image_upload'));
            modal.show();
        });
    });

    // Handle image upload
    document.getElementById('upload_story_image_btn')?.addEventListener('click', function() {
        const form = document.getElementById('kt_form_story_image_upload');
        ...
        fetch('{{ route("admin.stories.upload-image") }}', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                bootstrap.Modal.getInstance(document.getElementById('kt_modal_story_image_upload')).hide();
